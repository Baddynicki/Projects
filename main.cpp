...
public:
    ...
    void interest_calc(long long phoneNum) {
        ...
        // Loop through the expenses and find the relevant user data
        for (const auto& expense : expenses) {
            if (expense["phoneNumber"] == phoneNum) {
                total_expense += expense["amount"].get<double>();
                monthlyIncome = expense["monthlyIncome"].get<double>();
            }
        }

        if (monthlyIncome == 0.0) {
            cout << "Monthly income not found for this user." << endl;
            return;
        }

        double leftover_savings = monthlyIncome - total_expense;
        cout << "Your savings amounting to ₹" << leftover_savings << endl;
