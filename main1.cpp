...
void printMenuBox(bool loggedIn, const long long &phone)
{
    ...
    else
    {
        ...
        cout << BLUE << "| " << GREEN << "4. Show Savings and interest" << RESET << setw(width - 2 - 23) << " " << BLUE << "|" << RESET << endl;
        cout << BLUE << "| " << GREEN << "5. Search" << RESET << setw(width - 2 - 9) << " " << BLUE << "|" << RESET << endl;
        cout << BLUE << "| " << GREEN << "6. Update" << RESET << setw(width - 2 - 9) << " " << BLUE << "|" << RESET << endl;
        cout << BLUE << "| " << GREEN << "7. Delete" << RESET << setw(width - 2 - 9) << " " << BLUE << "|" << RESET << endl;
        cout << BLUE << "| " << RED << "8. Logout" << RESET << setw(width - 2 - 9) << " " << BLUE << "|" << RESET << endl;
    }
...
void interest_calc(long long phoneNum) {
    ...
    if (!inputFile.is_open()) {
        cout << "No expense data found!" << endl;
        return;
    }
    ...
    try {
        ...
        cout << "Error parsing expenses JSON: " << e.what() << endl;
        inputFile.close();
        return;
    }
    inputFile.close();
    ...
    try {
        ...
        cout << "Error parsing user JSON: " << e.what() << endl;
        userFile.close();
        return;
    }
    userFile.close();
    ...
    // Find the user's monthly income in users.json
    for (const auto& user : userData) {
        if (user.contains("phoneNumber") && user["phoneNumber"] == phoneNum) {
            userFound = true;
            if (user.contains("monthlyIncome")) {
                monthlyIncome = user["monthlyIncome"].get<double>();
            } else {
                cout << "Monthly income not found for this user in users.json." << endl;
                return;
            }
            break; 
        }
    ...
    if (!userFound) {
        cout << "No data found for the given phone number in users.json." << endl;
        return;
    }
    ...
    for (const auto& expense : expenses) {
        if (expense.contains("phoneNumber") && expense["phoneNumber"] == phoneNum) {
            userFound = true;
            if (expense.contains("amount")) {
                total_expense += expense["amount"].get<double>();
            }
        }
    }

    if (!userFound) {
        cout << "No expense data found for the given phone number." << endl;
        return;
    }

    double leftover_savings = monthlyIncome - total_expense;
    cout << "Your savings amount to ₹" << leftover_savings << endl;

    // Ask for interest rate and duration
    float interest_rate;
    int years;
    cout << "Enter interest rate in your bank (as percentage): ";
    cin >> interest_rate;
    cout << "Enter duration (in number of years): ";
    cin >> years;

    // Ensure input values are valid
    if (interest_rate <= 0 || years <= 0) {
        cout << "Invalid interest rate or duration." << endl;
        return;
    }

    // Compound interest calculation
    double amount = leftover_savings * pow(1 + interest_rate / 100, years);
    cout << "Total amount after interest is: ₹" << amount << endl;
}
...
int main()
{
    ...
    while (true)
    {
        ...
        case 4:
            if (loggedIn)
            {
                user.interest_calc(phoneNumber);
            }
            break;
        ...
        case 7:
            ...
            {
                string id;
                cout << "Enter the id of Expenses to delete one or enter * for delete all";
                getline(cin, id);
